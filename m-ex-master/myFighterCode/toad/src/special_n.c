...
/*
* Most of this code is just
* stolen from the mario example
* and that's okay.
* The only issue is that there
* are still fire particles
* whenever mario is loaded
* (or doc).  I don't know why.
*/
...
void SpecialN(GOBJ *fighter)
{
	...
	// clear subaction flags used by this function - this is the projectile flag used in HSDRawViewer
	...
	// set the accessory callback for toad's iceball
	// this function will spawn the iceball when the projectile flag is set
...
void SpecialAirN(GOBJ *fighter)
{
	...
	// set the accessory callback for toad's iceball
	// this function will actually spawn the iceball
	fighter_data->cb.Accessory4 = SpecialN_SpawnFireballThink;
	return;
}
...
/// 0x8029b7c0
///
void IS_TOADFireballSpawn(GOBJ *gobj)
{
	ItemData *item_data = gobj->userdata;

	// get the iceball custom attributes
	IceballAttr *attributes = item_data->itData->param_ext;

	float speed = attributes->speed;
	float angle = attributes->angle;

	// set velocity of iceball accordint to params
...
void SpecialN_SpawnFireballThink(GOBJ *gobj)
{
	...
	if (fighter_data->flags.throw_1 != 0)
	{
		...
		int fireball_id = MEX_GetFtItemID(gobj, MEX_ITEM_ICEBALL);
		CreateFireball(gobj, &bone_position, fireball_id, fighter_data->facing_direction);

		// create fireball e	ffect
		Effect_SpawnSync(MEX_EFFECT_FIREBALL, gobj, fighter_data->bones[bone_index].joint, &fighter_data->facing_direction);
	}
	return;
}

...
//Unchanged from mario example p much
...
void SpecialAirHi(GOBJ *gobj)
{
	...
	fighter_data->phys.self_vel.X = fighter_data->phys.self_vel.X * tdAttr->specialHi_initial_x_momemtum;
...
void SpecialHi_AnimationCallback(GOBJ *gobj)
{
	...
	{
		Fighter_EnterSpecialFall(gobj, 0, 1, 0, tdAttr->specialHi_fall_air_mobility, tdAttr->specialHi_landing_lag);
	}
	return;
}
///
...
void SpecialHi_IASACallback(GOBJ *gobj)
{
	...
	float stick_threshold = tdAttr->specialHi_stick_threshold;
	...
	{
		float new_angle = tdAttr->specialHi_stick_control * ((lstick_x_clamp - stick_threshold) / (1 - stick_threshold));
	...
	if (fighter_data->flags.throw_release != 0)
	{
		...
		if (tdAttr->specialHi_stick_reverse_threshold < lstick_x)
		{
...
void SpecialHi_OnLand(GOBJ *gobj)
{
	...
	Fighter_EnterSpecialLanding(gobj, 0, tdAttr->specialHi_landing_lag);
...
void SpecialAirHi_PhysicCallback(GOBJ *gobj)
{
	...
	{
		Fighter_PhysAir_ApplyGravity(fighter_data, tdAttr->specialHi_initial_gravity, fighter_data->attr.terminal_velocity);
	...
	{
		Fighter_Phys_UseAnimPosAndStick(gobj);
		fighter_data->phys.self_vel.X *= tdAttr->specialHi_initial_y_momemtum;
		fighter_data->phys.self_vel.Y *= tdAttr->specialHi_initial_y_momemtum;
		fighter_data->phys.self_vel.Z *= tdAttr->specialHi_initial_y_momemtum;
	}
	return;
}
///
